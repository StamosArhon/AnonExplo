...
$cases = @(
    ...
    @{Name='search build network'; Vpn=$false; Mutate={param($c) $c.services.'search-provider'.build.network='host' }},
    @{Name='broad build context'; Vpn=$false; Mutate={param($c) $c.services.'search-provider'.build.context='.' }},
    @{Name='remote repair image'; Vpn=$false; Mutate={param($c) $c.services.'search-provider'.pull_policy='always' }},
    @{Name='unreviewed build args'; Vpn=$false; Mutate={param($c) $c.services.'search-provider'.build | Add-Member NoteProperty args @{} }},
