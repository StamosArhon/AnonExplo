...
- The default SearXNG engine list is intentionally curated through `SEARCH_ENGINES` to reduce repeated failures from the broad inherited engine catalogue. With `SEARCH_CATEGORIES=auto`, news-engine names are kept out of ordinary queries and added for current/news-like queries. Clear it only when the operator accepts wider engine coverage and more upstream variability.
