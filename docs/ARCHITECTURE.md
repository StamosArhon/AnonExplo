...
Optional browser address-bar integration is a host-level operator setup documented in `docs/BROWSER_SEARCH_INTEGRATION.md`. It keeps the repo-managed SearXNG route on `127.0.0.1:8085` and points browser profiles at a separate localhost redirector on `127.0.0.1:8095` when DuckDuckGo fallback is desired.
