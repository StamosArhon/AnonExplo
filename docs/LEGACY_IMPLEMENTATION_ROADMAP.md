# Legacy Implementation Roadmap (historical, superseded 2026-09-10)

This preserves the former LLM-product history, not current instructions. Use
IMPLEMENTATION_ROADMAP.md; old model next steps and external fallback are superseded.
...
- Added a bounded multi-part query fan-out for grounded searches, adaptive general/news category and engine routing, curated SearXNG engine defaults, safer malformed-result handling, and explicit reporting of unresponsive upstream engines.
- Tuned the bundled SearXNG profile with bounded outgoing timeouts, connection pooling, HTTP/2, and a focused keep-only catalogue covering general web, news, Wikipedia, and research engines instead of the broad inherited catalogue.
...
- Added `scripts/setup-browser-search.ps1` as a repo-managed Windows setup entrypoint that generates the machine-local redirector and startup helpers, registers scheduled tasks or current-user Startup folder launchers, discovers Brave and Helium profiles, and applies the verified Chromium search-engine default flow.
- Updated the browser-search setup path so the redirector and stack startup use hidden VBS launchers, existing scheduled tasks can be repaired in place, and Docker Desktop's user logon entry can be repointed to a hidden `docker desktop start --detach` launcher instead of opening the dashboard in the foreground.
- Expanded the default SearXNG general-search catalogue with validated `bing` coverage alongside `brave`, while retaining separate news and research engines; tested `qwant`, `mojeek`, `google`, and `yahoo` from this workstation and kept them out of the default because they returned access-denied, empty, or protocol-failure responses.
...
- Browser search setup helpers generated by `scripts/setup-browser-search.ps1` should remain machine-local under `%LOCALAPPDATA%`; browser profiles, cookies, search history, and local helper output must not be committed.
...
- Browser-search startup entries must use hidden launchers. Do not point Task Scheduler directly at `node.exe`, and do not start Docker Desktop by launching `Docker Desktop.exe` directly from the AnonExplo startup path.
- The default SearXNG profile should keep at least two tested general sources where possible. Each additional upstream receives the query and adds latency/privacy exposure; provider-specific API integrations require a separate explicit privacy review.
...
- `scripts/validate.ps1`: first run on 2026-08-30 during `stamos/search-quality-tuning` reached the backend test stage but failed in the newly added SearXNG category test because its mocked HTTP response lacked a request object; the fixture was corrected before the passing rerun.
- `scripts/setup-browser-search.ps1 -SkipBrowserConfiguration`: passed on 2026-05-16 during `stamos/background-browser-search-startup`; regenerated local helper files, repaired existing scheduled tasks to use hidden VBS launchers, updated the current user's Docker Desktop logon entry to use a hidden `docker desktop start --detach` launcher, and restarted the redirector without touching browser profiles.
- Manual redirector restart simulation on 2026-05-16 during `stamos/background-browser-search-startup`: stopped the managed `search-fallback.js` Node process, restarted it through the `AnonExplo Search Fallback Redirector` scheduled task, and confirmed `http://127.0.0.1:8095/search?q=restart-through-task-check` redirected to SearXNG on `127.0.0.1:8085`.
- `$env:UI_PORT="3001"; scripts/validate.ps1`: failed once on 2026-05-16 during `stamos/background-browser-search-startup` after builds and unit tests because the host UI reachability probe hit a transient gateway-to-recreated-UI connection refusal while the gateway container was reused.
- `$env:UI_PORT="3001"; scripts/validate.ps1`: passed on rerun from a clean Compose state on 2026-05-16 during `stamos/background-browser-search-startup`; validation covered Compose config, `llama.cpp` profile config, Compose hardening policy, Docker image builds for `ui`, `backend`, and `fetcher`, backend unit tests, fetcher unit tests, UI server syntax, UI client syntax, and base-stack smoke/host reachability on UI `3001`, backend `8000`, and SearXNG `8085`; optional model-runtime probing was skipped because `Qwen2.5-7B-Instruct.Q4_K_M.gguf` was not present in `data\models`.
- `scripts/setup-browser-search.ps1 -SkipTaskRegistration -NoStartNow -SkipBrave -SkipHelium`: passed on 2026-05-16 during `stamos/browser-search-setup-script`; generated the local helper files without touching browser profiles or startup registration.
- Generated helper validation on 2026-05-16 during `stamos/browser-search-setup-script`: `node --check` passed for the generated redirector and Chromium configuration helpers under `%LOCALAPPDATA%\AnonExplo\search-fallback`, and the generated startup PowerShell helper parsed successfully.
- `scripts/setup-browser-search.ps1 -SkipBrowserConfiguration -NoStartNow`: passed on 2026-05-16 during `stamos/browser-search-setup-script`; on the current workstation, overwriting the existing root-level scheduled tasks returned `Access is denied`, and the script handled that by leaving the existing matching tasks in place.
- `$env:UI_PORT="3001"; scripts/validate.ps1`: passed on 2026-05-16 during `stamos/browser-search-setup-script`; validation covered Compose config, `llama.cpp` profile config, Compose hardening policy, Docker image builds for `ui`, `backend`, and `fetcher`, backend unit tests, fetcher unit tests, UI server syntax, UI client syntax, and base-stack smoke/host reachability on UI `3001`, backend `8000`, and SearXNG `8085`; optional model-runtime probing was skipped because `Qwen2.5-7B-Instruct.Q4_K_M.gguf` was not present in `data\models`.
...
- Use `scripts/setup-browser-search.ps1` for that optional browser path on new Windows devices. It generates local helper files under `%LOCALAPPDATA%`, can fall back to Startup folder launchers if scheduled-task registration is unavailable, and should be run with browsers closed or with `-ForceCloseBrowsers` when applying profile changes.
- If reboot causes a terminal or Docker Desktop dashboard to appear, rerun `scripts/setup-browser-search.ps1 -SkipBrowserConfiguration` to repair local startup entries without touching browser profiles.
